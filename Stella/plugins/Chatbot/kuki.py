...
@bot.on_message(
    filters.command(["addchat", f"addchat@Galactus_robot"])
)
...
@bot.on_message(
    filters.command(["rmchat", f"rmchat@Galactus_robot"])
)
...
async def kuki(_, message):
    try:
        ...
        if "Galactus" in text or "galactus" in text or "GALACTUS" in text:
        ...
        await bot.send_message(-1001175645313 , f"error in chatbot:\n\n{e}")
